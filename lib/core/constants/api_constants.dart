class ApiConstants {
  // when you run the back-end localy
  // if you use real device uncomment the line 4 and put your ip on laptop using ifconfig/ipconfig + run the server using this command: php artisan serve --host 0.0.0.0 --port 8000 +  make sure that you phone and laptop are on the same wifi or network source.
  // static const String baseUrl = "http://192.168.1.42:8000/api";

  // if you use emulator to run the app comment the line 4 and uncomment line 7
  // static const String baseUrl = "http://10.0.2.2:8000/api";

  // to run it as apk on real device: run this in back-end + uncomment this base url :
  static const String baseUrl = "http://localhost:8000/api";

// Auth
  static const String loginUrl = "/login";
  static const String registerUrl = "/register";
  static const String verifyOtpUrl = "/verify-otp";
  static const String logoutUrl = "/logout";

// Complaints
  //create complaint
  static const String addComplaintUrl = "/store/complaint";
  static const String getGovEntitiesUrl = "/government-entities";
  static const String getComplaintTypesUrl = "/complaint-types";
  static const String addAttachmentsUrl = "/addAttachments/";

  //get coplaints
  static const String getComplaintsUrl = "/filter_complaint_status";
}

        ...
        ElevatedButton.icon(
          onPressed: () async {
            ...
            if (result != null && result.files.isNotEmpty) {
              ...
              final message = provider.successMessage ?? provider.errorMessage ?? 'تمت العملية';
...
Widget _buildAttachmentCard(BuildContext context, String file) {
  ...
  if (file.startsWith("http")) {
    if (file.contains("127.0.0.1") || file.contains("localhost")) {
      ...
      fullUrl = file.replaceFirst(RegExp(r'127\.0\.0\.1|localhost'), '10.0.2.2');
    }
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => Scaffold(
          ...
          body: SfPdfViewer.network(normalizedUrl), // ←  normalizedUrl
        ),
      ),
    );
  } else if (isImage) {
